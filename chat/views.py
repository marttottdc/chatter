...
from .models import Room, ChatSession
...
@login_required
def chat_session(request, uuid):
    session = ChatSession.objects.get(uuid=uuid)

    if session.status == Room.WAITING:
        session.status = Room.ACTIVE
        session.agent = request.user
        session.save()

    return render(request, 'chat/sessions.html', {
        'session': session
    })
